...
export default function ProphecyDetailScreen() {
  ...
  function confirmDelete() {
    Alert.alert(t('prophecies.deleteTitle', { title: current.title }), t('prophecies.deleteBody'), [
        ...
        <Card className="rounded-2xl">
          <ProphecyFulfillmentToggle
            ...
            onChange={(fulfilledAt) =>
              void updateProphecy.mutateAsync({ id: prophecy.id, input: { fulfilledAt } })
            }
          />
        </Card>
      ...
      <ProphecyFulfillmentSheet
        ...
        onSave={async (values) => {
          if (fulfillmentSheet && fulfillmentSheet !== 'new') {
            await updateFulfillment.mutateAsync({ id: fulfillmentSheet.id, input: values });
          } else {
