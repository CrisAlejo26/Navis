...
export default function CalendarScreen() {
    ...
    const [festivos, setFestivos] = useState<Map<string, { name: string; scope: string }>>(
        new Map(),
    );
        ...
        () =>
            (schedule?.days ?? []).map((day) => ({
                ...
                meetings: day.meetings.filter(
                    (meeting) =>
                        sedesFiltro.length === 0 || sedesFiltro.includes(meeting.congregationId),
                ),
            })),
                ...
                view === 'month' ? (
                    <View className="min-h-0 px-4 pb-4 flex-1">
                        <CalendarMonth
                            ...
                            today={hoy}
                            meetingsOf={(date) =>
                                diasFiltrados.find((one) => one.date === date)?.meetings ?? []
                            }
