        ...
        <View className="flex-1 bg-background" style={{ paddingTop: insets.top + 8 }}>
            ...
            <View className="gap-3 px-4 pb-2">
                ...
                <BelieversFilters
                    ...
                    ministries={(ministries.data ?? []).map((one) => ({
                        id: one.slug,
                        name: one.name,
                    }))}
                />
                ...
                <FlatList
                    ...
                    renderItem={({ item, index }) => (
                        <BelieverCard
                            believer={item}
                            congregationName={
                                congregations.data?.find((one) => one.id === item.congregationId)
                                    ?.name ?? null
                            }
                    ...
                    ListFooterComponent={
                        isFetchingNextPage ? (
                            <Text className="py-3 text-xs text-center text-muted-foreground">
                                …
                            </Text>
                        ) : null
                    }
            ...
            >
                <Select
                    ...
                    options={(congregations.data ?? []).map((one) => ({
                        value: one.id,
                        label: one.name,
                    }))}
