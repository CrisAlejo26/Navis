    ...
    return (
        <View className="gap-3">
            <View className="gap-2 flex-row items-center justify-center">
                ...
                />
                <Text className="text-base font-sans-semibold text-foreground tabular-nums">
                    {year}
                </Text>
                <IconButton
...
function MiniMonth({ month, byDay }: { month: string; byDay: Map<string, NoteDay> }) {
    ...
    return (
        <View
            className="gap-1 p-2 rounded-xl border border-border bg-card"
            style={{ width: '31%' }}
        >
            <Text className="font-sans-medium text-[10px] text-muted-foreground uppercase">
                {label}
            </Text>
