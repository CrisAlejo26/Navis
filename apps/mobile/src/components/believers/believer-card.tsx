            ...
            <View className="gap-0.5 min-w-0 flex-1">
              ...
              {believer.phone ? (
                <View className="gap-1 flex-row items-center">
                  ...
                  <Text className="text-xs text-foreground tabular-nums" numberOfLines={1}>
                    {believer.phone}
                  </Text>
                </View>
              ) : (
...
function useAccentColor(): (accent: string) => string {
  const palette = themeColorsHex[useThemeStore((state) => state.resolvedTheme)];
  return (accent) =>
    accent.startsWith('#') ? accent : (palette[accent as keyof typeof palette] ?? palette.primary);
}
  ...
  if (gifts.length === 0 && slugs.length === 0 && tags.length === 0) {
    return (
      <View className="h-6 flex-row items-center">
        <Text className="text-[11px] text-muted-foreground">{t('believers.noClassification')}</Text>
      </View>
    );
  }
