              ...
              >
                <View className="gap-1 flex-row items-center">
                  <Text
                    className={cn(
                      'text-sm font-sans',
                      esHoy ? 'font-sans-semibold text-brand' : 'text-foreground',
                      holidayOf(day) && !esHoy && 'text-destructive',
                    )}
                  >
                    {Number(day.slice(8, 10))}
                  </Text>
                  {holidayOf(day) ? <View className="h-1 w-1 rounded-full bg-destructive" /> : null}
                </View>
                <View className="gap-0.5">
                  {meetings.slice(0, 2).map((meeting, index) => (
                    <View
                      ...
                      style={{ backgroundColor: accentHex(meeting.accent, palette) }}
                    />
                  ))}
                  {meetings.length > 2 ? (
