...
describe('el calendario en local (RFC 0002)', () => {
  ...
  function semanaDesde(desde: number): { viernes: string; dias: string[] } {
    const dias = [...Array(8)]
      .map((_one, index) =>
        new Date(Date.now() + (desde + index) * 24 * 60 * 60 * 1000).toISOString().slice(0, 10),
      )
  ...
  it('el tramo expande los patrones al vuelo: viernes propuesto con sus fases y huecos', async () => {
    ...
    expect(reunion?.slots.every((slot) => slot.believer === null && slot.id === null)).toBe(true);
  });
  ...
  it('crear un calendario con plantilla siembra la semana de esa labor en la sede', async () => {
    const vigilancia = await createCalendar(churchId, { name: 'Guardia', ministry: 'vigilancia' });
  ...
  it('crear una iglesia nace con calendarios y semana sin problemas', async () => {
    const otra = await createChurch({ name: 'Iglesia del Norte', city: 'Benidorm', ownerId: 'u2' });
    ...
    const dia = (
      await calendarRange(otra.id, pulpito.id, addMonths(mes, 4), endOfMonth(addMonths(mes, 4)))
  ...
  it('la reunión materializada conserva su nombre aunque se cambie el patrón después (D7)', async () => {
    ...
    const siguiente = new Date(new Date(`${viernes}T12:00:00Z`).getTime() + 7 * 24 * 60 * 60 * 1000)
      .toISOString()
      .slice(0, 10);
  ...
  it('los avisos salen de lo materializado: hueco y repetido, no propuestas', async () => {
    ...
    // pierde en el Excel.
    expect(summary.warnings.some((one) => one.kind === 'unassigned' && one.date === viernes)).toBe(
      true,
    );
