...
export class HabitsService {
    constructor(
        ...
        @InjectRepository(HabitReminderTag)
        private readonly reminderTags: Repository<HabitReminderTag>,
    ...
    private async setTags(habitId: string, tagIds: string[]): Promise<void> {
        ...
        if (tagIds.length > 0) {
            await this.habitTags.save(
                tagIds.map((tagId) => this.habitTags.create({ habitId, tagId })),
            );
        }
    }
