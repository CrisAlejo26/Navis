...
/**
 * El rol **predicador de apoyo** (RFC 0014): el día a día del pastor
 * —calendario, creyentes, listas, comunicaciones— sin `users.manage` ni
 * `churches.manage`, así que nunca crea ni entra en más de una iglesia.
 *
 * Misma idea que `SeedMinistryRoles`: en una instalación nueva, `CreateRoles`
 * ya lo habrá sembrado a partir de `ROLES` (que ya lo incluye), así que aquí
 * solo se le ponen el nivel y los permisos; en una que ya existía, no está
 * todavía y se inserta.
 */
const SLUG = 'predicador-apoyo';

export class SeedPredicadorApoyo1788307200000 implements MigrationInterface {
  name = 'SeedPredicadorApoyo1788307200000';
