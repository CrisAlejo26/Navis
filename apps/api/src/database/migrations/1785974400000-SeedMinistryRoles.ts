...
export class SeedMinistryRoles1785974400000 implements MigrationInterface {
        ...
        for (const [before, after] of RENAMES) {
            ...
            if (await idOf(after)) {
                ...
                await queryRunner.query(
                    `UPDATE "roles" SET "slug" = ${mark(1)} WHERE "id" = ${mark(2)}`,
                    [after, oldId],
                );
            }
            ...
            if (hasUsers) {
                await queryRunner.query(
                    `UPDATE "user" SET "role" = ${mark(1)} WHERE "role" = ${mark(2)}`,
                    [after, before],
                );
            }
        }
    ...
    async down(queryRunner: QueryRunner): Promise<void> {
        ...
        for (const [before, after] of RENAMES) {
            await queryRunner.query(
                `UPDATE "roles" SET "slug" = ${mark(1)} WHERE "slug" = ${mark(2)}`,
                [before, after],
            );
            if (hasUsers) {
                await queryRunner.query(
        ...
        for (const slug of ['biblias', 'sonido', 'pulpito']) {
            ...
            if (hasUsers) {
                await queryRunner.query(
                    `UPDATE "user" SET "role" = 'member' WHERE "role" = ${mark(1)}`,
                    [slug],
                );
            }
        }
