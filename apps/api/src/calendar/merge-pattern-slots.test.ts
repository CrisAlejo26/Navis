...
const slot = (id: string, name: string, position: number, peopleCount = 0) =>
    ({ id, name, position, peopleCount, note: null }) satisfies SlotLike;
...
describe('mergeSlots', () => {
    ...
    it('crea vacía la fase nueva y conserva lo asignado', () => {
        const plan = mergeSlots(
            [slot('a', 'Intro', 0, 1), slot('b', 'Predicación', 1)],
            ['Intro', 'Predicación'],
    ...
    it('quita una fase retirada del patrón solo si está vacía', () => {
        const plan = mergeSlots(
            [slot('a', 'Intro', 0), slot('b', 'Cierre', 1, 2)],
            ['Intro', 'Cierre'],
            ['Nueva'],
        );
    ...
    // Una fase con varias personas cuenta como rellenada igual que con una.
    it('conserva una fase retirada del patrón que ya lleva a varias personas', () => {
        const plan = mergeSlots([slot('a', 'Testimonios', 0, 3)], ['Testimonios'], ['Otra']);
