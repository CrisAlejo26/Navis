    ...
    manager(): EntityManager {
        ...
        return {
            find: (entidad: unknown, opciones?: { where?: Record<string, unknown> }) =>
                Promise.resolve(
                    this.tabla(entidad).filter((fila) => coincide(fila, opciones?.where)),
                ),
            ...
            softRemove: (filas: Row | Row[]) => {
                for (const fila of Array.isArray(filas) ? filas : [filas])
                    fila.deletedAt = new Date();
