...
export class CreateTableViewDto {
    ...
    @ApiPropertyOptional({ type: [RowFilterDto], description: 'Los filtros con los que se miraba' })
    @IsOptional()
    @IsArray()
    @ArrayMaxSize(30)
    ...
    @ApiPropertyOptional({ description: 'La key de la columna por la que se ordenaba' })
    @IsOptional()
    @IsString()
    sortBy?: string | null;
