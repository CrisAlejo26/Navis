...
export class BelieversService {
    ...
    async create(churchId: string, input: CreateBelieverInput): Promise<Believer> {
        const believer = await this.believers.save(
            this.believers.create({
                ...
                alertAfterDays:
                    input.alertAfterDays === undefined
                        ? DEFAULT_ALERT_AFTER_DAYS
        ...
        await this.links.setMinistries(
            believer.id,
            input.ministries ?? [],
            input.ministryDates ?? {},
        );
        await this.links.setGifts(
            churchId,
            believer.id,
            input.giftIds ?? [],
            input.giftDates ?? {},
    ...
    async update(churchId: string, id: string, input: UpdateBelieverInput): Promise<Believer> {
        ...
        if (input.vivenciasReadings !== undefined)
            believer.vivenciasReadings = input.vivenciasReadings;
    ...
    async setCongregation(
        ...
        const result = await this.believers.update(
            { id: In(unique), churchId },
            { congregationId },
        );
