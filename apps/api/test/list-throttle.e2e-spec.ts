...
describe('El freno de la puerta (e2e)', () => {
    ...
    beforeAll(async () => {
        ...
        await post('/api/v1/churches', { name: `Iglesia Freno ${sello}`, city: 'Elda' }).expect(
            201,
        );
        ...
        token =
            body<ListShareState>(
                await post(`/api/v1/lists/${lista}/share`, { visibility: 'restricted' }).expect(
                    201,
                ),
            ).shareToken ?? '';
    });
    ...
    it('al undécimo intento desde el mismo origen contesta 429 y dice cuánto falta', async () => {
        ...
        for (let vez = 0; vez < LIST_TRIES; vez += 1) {
            const respuesta = await intentar(
                vez % 2 === 0 ? `203.0.113.${String(vez)}` : undefined,
            );
