...
describe('Profecías personales (e2e)', () => {
    ...
    it('empieza en espera y pasa a «en camino» al anotar un cumplimiento', async () => {
        ...
        const fila = body<Paginated<ProphecyListItem>>(enCamino).items.find(
            (one) => one.id === casa,
        );
    ...
    describe('exportar (RFC 0009)', () => {
        it('devuelve las filas con el cuerpo entero, no con el extracto', async () => {
            ...
            expect(
                salida.rows.every((row) => typeof row.body === 'string' && row.body.length > 0),
            ).toBe(true);
    ...
    describe('la barrera del dueño (D1)', () => {
        it('otro usuario no ve ninguna de mis profecías en su listado', async () => {
            const page = body<Paginated<ProphecyListItem>>(
                await login(deOtro).get('/api/v1/prophecies'),
            );
        ...
        it('tampoco la puede editar, borrar, ni anotar nada en ella', async () => {
            const editar = await login(deOtro).patch(`/api/v1/prophecies/${casa}`, {
                title: 'Mía',
            });
