...
describe('Accesos a listas (e2e)', () => {
    ...
    beforeAll(async () => {
        ...
        await post('/api/v1/churches', { name: `Iglesia Puerta ${sello}`, city: 'Elda' }).expect(
            201,
        );
        ...
        for (const [nombre, apellido] of [
            ...
            const persona = body<Believer>(
                await post('/api/v1/believers', { firstName: nombre, lastName: apellido }).expect(
                    201,
                ),
            );
    ...
    it('publicar en restringida sin conceder a nadie se rechaza y lo explica (§7.1)', async () => {
        const roto = await post(`/api/v1/lists/${pulpito}/share`, {
    ...
    it('concede y publica las dos listas, cada una a un acceso distinto (D19)', async () => {
        ...
        tokenPulpito =
            body<ListShareState>(
                await post(`/api/v1/lists/${pulpito}/share`, { visibility: 'restricted' }).expect(
                    201,
                ),
            ).shareToken ?? '';
        tokenSonido =
            body<ListShareState>(
                await post(`/api/v1/lists/${sonido}/share`, { visibility: 'restricted' }).expect(
                    201,
                ),
            ).shareToken ?? '';
    ...
    it('usuario que no existe y contraseña mala dan el mismo mensaje (D26)', async () => {
        ...
        expect(body<{ message: string }>(malaClave).message).toBe(
    ...
    it('**un acceso concedido a una lista no abre la otra**: 403 con su mensaje (D26)', async () => {
        ...
        await anon()
            .get(`/api/v1/public/lists/${tokenSonido}`)
    ...
    it('quien ya entró abre las demás suyas sin escribir nada (D23)', async () => {
        ...
        await anon()
            ...
            .expect(200);
        await anon()
            .get(`/api/v1/public/lists/${tokenPulpito}`)
            .set('Cookie', sesionJuan)
            .expect(200);
    });
    ...
    it('las estadísticas dicen quién entró y cuántos no han entrado nunca (D35)', async () => {
        ...
        expect(
            stats.access.recent.every(
                (one) =>
                    one.ipPrefix.endsWith('.0') ||
                    one.ipPrefix.endsWith('::') ||
                    one.ipPrefix === '',
            ),
        ).toBe(true);
    });
