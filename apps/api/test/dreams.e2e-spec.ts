...
describe('Sueños personales (e2e)', () => {
    ...
    it('el sueño de otra persona no existe para quien pregunta (D1)', async () => {
        ...
        expect(
            (await login(deOtro).patch(`/api/v1/dreams/${puerta}`, { title: 'Mío' })).status,
        ).toBe(404);
        ...
        expect(body<Paginated<DreamListItem>>(suyos).items.map((one) => one.id)).not.toContain(
            puerta,
        );
    ...
    it('sin sesión no se llega a nada', async () => {
        ...
        expect((await request(app.getHttpServer()).get('/api/v1/dreams/emotions')).status).toBe(
            401,
        );
    });
