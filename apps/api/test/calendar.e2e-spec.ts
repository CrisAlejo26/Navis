...
describe('Calendario (e2e)', () => {
    ...
    it('cada iglesia nace con una sede, y se puede añadir otra', async () => {
        ...
        expect(body<Congregation>(creada).accent).not.toBe(
            body<Congregation[]>(inicial)[0]?.accent,
        );
    });
    ...
    it('cada sede nace con la semana de serie, y el mes se llena solo', async () => {
        ...
        const semana = body<MeetingPattern[]>(respuesta).filter(
            (one) => one.congregationId === elda,
        );
    ...
    it('los candidatos vienen paginados, con quien lleva más tiempo sin subir primero', async () => {
        const pagina = await request(app.getHttpServer())
            .get(
                `/api/v1/calendars/${calendarId}/preachers?from=${rango.from}&to=${rango.to}&all=true`,
            )
            .set('Cookie', cookie)
            .expect(200);
