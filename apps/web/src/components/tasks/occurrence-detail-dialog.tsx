...
export function OccurrenceDetailDialog({
    ...
    const isPending =
        occurrence.kind === 'task' ? setTaskStatus.isPending : setHabitStatus.isPending;

    const toggle = () => {
        const status = completed ? 'pendiente' : 'completada';
        const promise =
            occurrence.kind === 'task'
                ? setTaskStatus.mutateAsync({
                      taskId: occurrence.item.taskId,
                      date: item.date,
                      status,
                  })
                : setHabitStatus.mutateAsync({
    ...
    const remove = () => {
        ...
        void promise.then(() => {
            toast.success(
                occurrence.kind === 'task' ? t('tasks.removed') : t('tasks.habitRemoved'),
            );
            setDeleting(false);
            onClose();
        });
                    ...
                    <dl className="gap-x-4 gap-y-2 text-sm grid grid-cols-[auto_1fr]">
                        ...
                        {occurrence.kind === 'task' && (
                            <>
                                ...
                                <dd
                                    style={{
                                        color: `var(--color-${PRIORITY_ACCENT[occurrence.item.priority]})`,
                                    }}
