import { noteAudioPath, type NoteAudio } from '@navis/shared';
...
/**
 * Un audio de una nota, con el reproductor del navegador.
 *
 ...
 * y el guard de iglesia mira la cookie de sesión. Sin eso, el navegador pide el
 * fichero **sin cookie** y recibe un 401.
 */
export function NoteAudioPlayer({ audio, className }: { audio: NoteAudio; className?: string }) {
  ...
  return (
    <span className={cn('gap-2 min-w-0 flex items-center', className)}>
      <audio
        ...
        src={`${api.baseUrl}${noteAudioPath(audio.id)}`}
      ...
      <span className="shrink-0 text-[11px] text-muted-foreground tabular-nums">
        {audio.recorded ? t('notes.audio.recorded') : t('notes.audio.attached')}
