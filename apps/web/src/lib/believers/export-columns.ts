...
export function useBelieverExportColumns(catalogs: {
    ...
    return [
        {
            key: 'firstName',
            header: t('believers.firstName'),
            value: (row) => cellText(row.firstName),
        },
        {
            key: 'lastName',
            header: t('believers.lastName'),
            value: (row) => cellText(row.lastName),
        },
        {
            key: 'status',
        ...
        },
        {
            ...
            value: (row) =>
                cellTags(row.gifts.map((gift) => ({ text: gift.name, accent: gift.accent }))),
        },
        {
            key: 'tags',
            header: t('believerTags.title'),
            value: (row) =>
                cellTags(row.tags.map((tag) => ({ text: tag.name, accent: tag.accent }))),
        },
