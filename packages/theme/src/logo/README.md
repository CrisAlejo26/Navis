...
| `blanco-con-fondo.svg` | Referencia del diseñador. **El build no la usa**: compone el fondo azul él mismo para controlar el encuadre |
...
## El encuadre no es el del SVG

Estos SVG traen mucho margen: el barco ocupa un 69 % de ancho y un 62 % de
alto del lienzo, y está descentrado. Servido tal cual, en un favicon de 16 px
el dibujo se queda en nada.

Por eso `scripts/brand-logo.mjs` **mide** el dibujo (rasterizando y buscando
los píxeles opacos) y lo reencuadra centrado, con la proporción que pida cada
destino: casi todo el cuadro en el favicon, un 72 % en los iconos de
aplicación, y bastante menos en los maskable y adaptativos, donde el sistema
recorta y hay que dejar zona segura.

Si mañana llega un logo con otros márgenes, el encuadre se ajusta solo.
