...
## En Navis (léelo antes de aplicar nada)

Esta copia está vendorizada en el repositorio (upstream `nextlevelbuilder/ui-ux-pro-max-skill`, v2.11.0). Antes de seguir el flujo genérico:

- **El stack ya está detectado.** No lo adivines: usa `--stack react` para `apps/web` y `--stack react-native` para `apps/mobile`. Este proyecto **no** es Next.js.
- **La paleta y la tipografía de la marca ya están decididas** y mandan sobre lo que devuelva la base de datos: los tokens semánticos viven en `packages/theme/src/tokens.css` y el azul de marca es `#2140cf`. Toma de esta skill *patrones, jerarquía, UX, accesibilidad y motion*; los colores concretos solo como inspiración, nunca sustituyendo los tokens (Regla 3).
- **Nada de cruces** en iconos, ilustraciones ni elementos decorativos (Regla 7), venga la sugerencia de donde venga.
- **`--persist` está desaconsejado** aquí: escribiría `design-system/` en la raíz y duplicaría lo que ya documentan `packages/theme` y los RFC de `docs/rfcs`. Si aun así lo usas, pasa `--output-dir` a un directorio temporal.
- En Windows, si `python` no está en el PATH, prueba `py -3`.
